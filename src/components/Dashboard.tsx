...
// 更新 Props 接口，接收所有预计算的数据
interface DashboardProps {
  schedule: BellSchedule[];
  ringtones: RingtoneType[];
  now: Date; // 接收实时时间
  sortedSchedule: BellSchedule[]; // 接收排序后的计划
  currentBell: BellSchedule | null; // 接收当前闹铃
  nextBell: BellSchedule | null; // 接收下一个闹铃
  countdownData: { text: string | null; remainingMinutes: number }; // 接收倒计时数据
  elapsedMinutes: number; // 接收已逝去分钟数
}
...
const Dashboard: React.FC<DashboardProps> = ({
  ...
  // Auto-scroll effect (保持不变，但依赖项现在都是来自 Props)
  useEffect(() => {
  ...
  }, [currentBell?.id, nextBell?.id, sortedSchedule]); // 依赖项现在是 props
